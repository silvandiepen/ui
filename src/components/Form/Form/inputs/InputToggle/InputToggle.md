...
<script setup>
import { ref } from 'vue'
import { InputToggle } from '@tiko/ui'
...
## Differences from InputCheckbox

While both components handle boolean values, they serve different purposes:
- **InputToggle**: For on/off settings, immediate effect
- **InputCheckbox**: For selections, typically requires form submission
...
- `InputRadio` - Single selection from options
- `InputSwitch` - Multi-option switch selector