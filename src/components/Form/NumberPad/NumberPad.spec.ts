...
describe('NumberPad.vue', () => {
  ...
  it('emits submit event when submit button is clicked', async () => {
    ...
    const submiButton = wrapper.find('.number-pad__button--submit')
    await submiButton.trigger('click')
  ...
  it('disables only submit button when disableSubmit is true', () => {
    ...
    expect(submiButton.attributes('disabled')).toBeDefined()
    expect(numberButton.attributes('disabled')).toBeUndefined()
  })
  ...
  it('uses custom submit icon when provided', () => {
    const wrapper = mount(NumberPad, {
      props: {
        submiIcon: 'custom-submit-icon'
      }
    })

    const submiButton = wrapper.find('.number-pad__button--submit')
    const icon = submiButton.find('.t-icon')
