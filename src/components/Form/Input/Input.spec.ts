...
describe('Input.vue', () => {
  ...
  it('displays prefix icon when provided', () => {
    ...
    expect(prefixIcon.findComponent({ name: 'Icon' }).props('name')).toBe('user')
  })
  ...
  it('displays suffix icon when provided', () => {
    ...
    expect(suffixIcon.findComponent({ name: 'Icon' }).props('name')).toBe('search')
  })
  ...
  it('generates unique input id', () => {
    const wrapper1 = mount(Input, { props: { label: 'Test 1' } })
    const wrapper2 = mount(Input, { props: { label: 'Test 2' } })
